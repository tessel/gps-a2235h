  ...
  self.packetizer.on('packet', function (packet) {
    ...
    if (packet[0] === '$') {
      ...
      if (DEBUG >= 2) {  //  pretty print
        // In this debug mode, invalid sentences that cause nmea.parse ()
        // to throw will be allowed to crash the program.
        datum = nmea.parse(packet);
        ...
        // In this mode (DEBUG = 0 or 1), thrown exceptions that occur
        // when parsing an invalid sentence will be swallowed.
        try {
          datum = nmea.parse(packet);
        } catch (error) {}
      }
